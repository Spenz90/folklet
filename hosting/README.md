...
From the Folklet application folder, after source setup:
...
The shell wrapper uses Folklet's bundled Node to run the installer. The installer validates its platform, runtime and target files, refuses differing existing settings and symbolic links, writes a systemd user unit/launcher/environment file, and reloads the current user's manager. The explicit systemctl command starts and enables Folklet.
...
The launcher keeps the service on Folklet's standard loopback ports, points optional browser downloads at the private data directory, and removes Node injection overrides. The service uses restrictive default file permissions, restarts a failed host and stops its own child processes as a group.
