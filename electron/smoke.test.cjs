...
test('desktop diagnostics are opt-in and require a report inside the isolated root plus owned host PID',t=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'crew-smoke-options-'));t.after(()=>fs.rmSync(root,{recursive:true,force:true}));
...
test('desktop smoke requires rendered application content and no renderer Node bridge',()=>{
 const page={title:'Crew',app:true,main:true,rendered:true,tokenPresent:true,nodeAbsent:true};assert.equal(validSmokePage(page),true);
...
test('desktop smoke reports booleans only, saves an isolated screenshot and exits',async t=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'crew-smoke-render-'));t.after(()=>fs.rmSync(root,{recursive:true,force:true}));let code;
 const report=path.join(root,'result.json'),page={title:'Crew',app:true,main:true,rendered:true,tokenPresent:true,nodeAbsent:true};
