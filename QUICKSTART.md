...
Crew runs on a computer or private server that you control. Your phone connects to that host. The Codex desktop app can be closed or absent.
...
Extract the whole release archive for your platform. On Windows, open **Start Crew.cmd**. On Mac, open **Crew.app**. On Linux, open **Start Crew.sh** inside the extracted Crew folder. If your file manager does not offer Run, open a terminal there and run `sh 'Start Crew.sh'`.
...
Starting from source instead? Follow the [source setup commands](README.md#build-from-source). Quit any existing Crew before setup, rebuilding or updating it.

## 2. Connect a model

Open **My workspace → Connections** on the host.

- **ChatGPT:** choose ChatGPT, start sign-in, and continue on OpenAI's website. Crew uses the bundled official Codex engine. Account eligibility and usage limits still apply.
...
- **A local model:** choose Ollama after installing and starting Ollama separately. Its default address is http://127.0.0.1:11434/v1 on the computer running Crew.

Leave **Remember this key on this computer** unchecked for a session-only API key. Check it only if you want the host to retain the key after Crew quits. Retained keys are stored as plaintext in Crew's private data folder, with access governed by the host's file permissions. See [Connections](PROVIDERS.md#keys-billing-and-limits) for storage details, costs and model limitations.
...
> Create a file called hello.txt in your workspace containing Hello from Crew. Tell me when it is saved.
...
Marking an app improvement reviewed does not change the app. The host's separate **App changes** workflow lets you prepare a source draft, review it, approve selected tests and explicitly apply an exact revision with backups. Run only trusted test code: Node restrictions are not a security sandbox, and network/local services remain reachable. Crew never applies or restarts automatically. Read [the feature guide](FEATURES.md) before using drafts, GitHub integrations, Telegram updates or fallback models.

## 6. Optional: connect your iPhone

1. Keep Crew running on the host. Install Tailscale on the host and iPhone, sign in to the same private network, and connect both devices.
...
4. Open Crew's displayed private address in **Safari** on the iPhone. On the host, create a pairing code and enter it on the phone. Codes expire after five minutes and work once.
...
Open **My workspace → Computer access** on the host. If supported and available, enable it for this session. Your bot must ask before every screenshot, click, keystroke or scroll. Screenshots can include other applications and go to the model you selected. Restarting Crew turns this off again.
...
| --- | --- |
| Crew cannot start, or port 4318 is occupied | Quit the other Crew instance or application using that port, then reopen Crew. Do not end unfamiliar processes. |
...
| Only Default reasoning is offered | For ChatGPT, choose a named model first. Other models or connections may have no verified adjustable levels in Crew. Default still lets them work normally. |
...
| The browser cannot start | Install a compatible browser; on Linux also check its system-library and sandbox requirements. Crew does not disable the browser sandbox to make an unsupported setup work. |
| Phone address does not load | Keep the host awake, connect Tailscale on both devices, and check the connection in Crew. Use the displayed HTTPS address, including port 8443. |
| Phone setup reports a conflicting service | Crew leaves that service intact. Review the existing Tailscale Serve configuration yourself before retrying. Never use public Funnel for Crew. |
...
Closing a packaged desktop window keeps Crew available; use **Quit Crew** to stop it. A source host stays running in its terminal, and a server service uses the stop command in the [hosting guide](HOSTING.md). Back up your private data before updates and keep it out of GitHub.
