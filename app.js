import {DraftStorage} from './draft-storage.mjs';
...
const draftStorage=new DraftStorage({storage:{getItem:key=>localStorage.getItem(key),setItem:(key,value)=>localStorage.setItem(key,value),removeItem:key=>localStorage.removeItem(key)}}),syncClient=new SyncClient();
let draftWorkspace=null;
function persistDrafts(){draftStorage.save(drafts,draftAttachments);if(draftStorage.error)toast(draftStorage.error);}
function restoreDrafts(){if(data.workspaceId&&draftWorkspace!==data.workspaceId){const changed=!!draftWorkspace;draftStorage.bind(data.workspaceId);const saved=draftStorage.load();drafts=changed?saved.drafts:{...saved.drafts,...drafts};draftAttachments=changed?saved.attachments:{...saved.attachments,...draftAttachments};draftWorkspace=data.workspaceId;if(changed){attachments=[];selected=null;channel=null;inspect=null;view=phoneMode?'phone-bots':'home';renderKey='';msgKey='';panelKey='';}}}
const connection=new ConnectionState({onChange:renderConnection});
function renderConnection(state=connection.snapshot()){
 const banner=$('connection-banner');if(!banner)return;
 const copy=connectionCopy(state);banner.hidden=state.status==='online';banner.dataset.status=state.status;
 const html='<span><b>'+esc(copy.title)+'</b><small>'+esc(copy.detail)+'</small></span>'+(state.status==='expired'?'<button class="btn" data-copy-draft>Copy draft</button>':'')+'<button class="btn" data-reconnect>'+esc(copy.action)+'</button>';
 if(banner.innerHTML!==html)banner.innerHTML=html;
 const chip=$('phone-host-status');if(chip){chip.dataset.status=state.status;chip.innerHTML='<i></i><span>'+esc(state.status==='online'?'Host connected':copy.title)+'</span><span aria-hidden="true">›</span>';}
}
...
function removeAttachment(index){const key=channel||selected;if(sendingDrafts.has(key)||!Number.isInteger(index)||index<0||index>=attachments.length)return;attachments.splice(index,1);draftAttachments[key]=[...attachments];renderAttachments();saveDraft();}
...
function saveDraft(){if($('prompt')){const key=channel||selected;drafts[key]=$('prompt').value;draftAttachments[key]=[...attachments];persistDrafts();}}
...
const phoneBackLabel=()=>({'phone-bots':'bots','phone-groups':'groups','phone-settings':'settings',inbox:'activity'}[history.state?.crewPhone?.returnView||phoneReturn]||'FOLKLET');
function phoneBack(){
...
async function api(route,body){let r;try{r=await fetch('/api/'+route,{method:body?'POST':'GET',headers:{'X-Crew-Token':window.CREW_TOKEN,'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(route==='state'||route.startsWith('state-delta')?15000:120000)});}catch{const error=Error(body?'The connection dropped before confirmation. Check task history before trying again; your request may have reached the host.':'FOLKLET cannot reach your host. Check your connection and keep FOLKLET running.');connection.failure(error);throw error;}let result;try{result=await r.json();}catch{throw Error('FOLKLET could not read the response. Check your connection and try again.');}if(!r.ok){const error=Object.assign(Error(result.error||'Request failed'),{code:r.status===401&&window.CREW_MOBILE?'PAIRING_REQUIRED':result.code});if(error.code==='CONNECTION_EXPIRED'||error.code==='PAIRING_REQUIRED')connection.failure(error);throw error;}return result;}
...
function header(){
 ...
 if(phoneMode){
  if(chat){
   ...
   const title={'phone-bots':'Bots','phone-groups':'Groups','phone-settings':'Settings',inbox:'Activity',routines:'Routines',tasks:'Tasks'}[view]||'FOLKLET';
...
function render(){
 ...
 renderPhoneNav();renderInspector();renderConnection();
}
...
function renderChat(){
 ...
 if(renderKey!==key){
  ...
  input.oninput=()=>{size();saveDraft();};size();
 ...
 if(nextKey!==msgKey){
  ...
  if(owners.some(owner=>(owner.messageCount||0)>owner.messages.length))el.insertAdjacentHTML('afterbegin','<button class="btn load-history" data-load-history>Load older messages</button>');
...
async function send(e){
 e.preventDefault();
 if(!connection.canSend()){saveDraft();toast('Reconnect before sending. Your draft stays in this open page.');return;}
 ...
 const draftKey=channel||selected,channelId=channel,workspace=data.workspaceId,files=[...attachments],button=$('send');
 ...
 try{
  ...
  if(data.workspaceId!==workspace)return;
  ...
  await refresh();
 }catch(e){toast(e.message);}finally{persistDrafts();delete input.dataset.sending;button.disabled=false;sendingDrafts.delete(draftKey);if((channel||selected)===draftKey&&$('send')){$('send').disabled=uploadingDrafts.has(draftKey);renderAttachments();}}
}
...
$('file-input').onchange=async e=>{const file=e.target.files[0],target=fileTarget;fileTarget=null;if(!file||!target?.owner){e.target.value='';return;}const {owner,draftKey}=target,workspace=data.workspaceId;if(draftKey){uploadingDrafts.set(draftKey,(uploadingDrafts.get(draftKey)||0)+1);if((channel||selected)===draftKey&&$('send'))$('send').disabled=true;}try{if(file.size>10000000)throw Error('Files must be 10 MB or smaller.');const base64=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result.split(',')[1]);r.onerror=()=>reject(Error('Could not read this file. Try choosing it again.'));r.readAsDataURL(file);});const r=await api('upload',{id:owner,name:file.name,base64});if(data.workspaceId!==workspace)return;if(draftKey){const attachment={owner,path:r.path};draftAttachments[draftKey]=[...(draftAttachments[draftKey]||[]),attachment];if((channel||selected)===draftKey&&$('attachments')){attachments.push(attachment);draftAttachments[draftKey]=[...attachments];renderAttachments();}}if(inspectTab==='files'&&inspect===owner)loadFiles(bot(owner));saveDraft();persistDrafts();toast('File added to workspace');}catch(e){toast(e.message);}finally{e.target.value='';if(draftKey){const count=(uploadingDrafts.get(draftKey)||1)-1;if(count)uploadingDrafts.set(draftKey,count);else uploadingDrafts.delete(draftKey);if((channel||selected)===draftKey&&$('send'))$('send').disabled=sendingDrafts.has(draftKey)||uploadingDrafts.has(draftKey);}}};
...
const refreshLoop=createRefreshLoop({refresh,connection,visible:()=>document.visibilityState==='visible'});
renderConnection();refreshLoop.start();setInterval(()=>{if(document.visibilityState==='visible'&&connection.canSend())screen();},3000);
...
function accountMenu(){
 modal('FOLKLET','Your workspace',`<div class="contact-actions"><button data-close-nav="home">${icon('home')} Manage bots</button><button data-close-nav="routines">${icon('clock')} All routines</button><button data-close-nav="tasks">${icon('board')} Task history</button><button data-crew-settings="models">${icon('settings')} Models & reasoning</button><button data-crew-settings="skills">${icon('file')} Skills</button><button data-crew-settings="plugins">${icon('computer')} Plugins</button><button data-crew-settings="recall">${icon('clock')} Past work</button><button data-crew-settings="learning">${icon('memory')} Learning review</button>${window.CREW_MOBILE?'':'<button data-crew-app-changes>'+icon('file')+' App changes</button><button data-crew-settings="integrations">'+icon('computer')+' Integrations</button><button data-crew-settings="notifications">'+icon('inbox')+' Private notifications</button>'}<button data-theme>${icon('theme')} Switch appearance</button>${window.CREW_MOBILE?'':'<button data-crew-settings="connections">'+icon('bot')+' Connections</button>'}<button data-phone>${icon('computer')} ${window.CREW_MOBILE?'This iPhone':'Connect iPhone'}</button><button data-crew-settings="computer">${icon('computer')} Computer access</button><button data-crew-settings="reliability">${icon('settings')} Data & reliability</button><button data-crew-settings="host">${icon('computer')} Host status</button><button data-crew-settings="cloud">${icon('home')} Always-on setup</button><button data-settings-guide="FEATURES.md">${icon('file')} Feature guide</button><button data-settings-guide="QUICKSTART.md">${icon('info')} Quick start</button></div>`,'account');
}
...
document.addEventListener('visibilitychange',()=>{saveDraft();refreshLoop.wake();});
window.addEventListener('pageshow',()=>refreshLoop.wake());
window.addEventListener('online',()=>refreshLoop.wake());
window.addEventListener('offline',()=>{saveDraft();connection.failure();});
document.addEventListener('click',event=>{if(!event.target.closest('[data-reconnect]'))return;if(connection.status==='expired'){saveDraft();location.reload();}else refreshLoop.wake();});
document.addEventListener('click',async event=>{if(!event.target.closest('[data-copy-draft]'))return;saveDraft();const text=$('prompt')?.value;if(!text){toast('Open the conversation with your draft first.');return;}try{await navigator.clipboard.writeText(text);toast('Draft copied. You can paste it after reconnecting. Recheck attachments before sending.');}catch{toast('Select and copy the draft text before reloading.');}});


let phonePoll=null;
async function phonePanel(pairing=null,{quiet=false}={}){
 clearTimeout(phonePoll);
 if(window.CREW_MOBILE){modal('FOLKLET on your iPhone','Connected to your computer.', '<p class="note">In Safari, open Share → Add to Home Screen, leave Open as Web App enabled if shown, and tap Add.</p><p class="note">Keep Tailscale connected on both devices and FOLKLET running on your awake computer. Your bots, chats, files and routines are shared.</p><button class="btn danger" data-phone-signout>Disconnect this iPhone</button>','phone');return;}
 if(!quiet)modal('Connect iPhone','Take your helpers with you.','<p class="note">Checking your connection…</p>','phone');
 try{
 ...
 const instructions=unix?'<a href="https://tailscale.com/download/'+(status.platform==='darwin'?'mac':'linux')+'" target="_blank" rel="noopener noreferrer">2. Install Tailscale on this '+(status.platform==='darwin'?'Mac':'computer')+' ↗</a><p class="note">Open Tailscale and sign in first. FOLKLET will then set up the private connection using your existing installation.</p><button class="btn primary" data-phone-setup '+(working?'disabled':'')+'>Set up connection</button>':'<button class="btn primary" data-phone-setup>2. Set up this PC</button><p class="note">The setup window installs Tailscale if needed and guides you through signing in. Windows may ask to allow the setup.</p>';
 const content=ready?'<p class="note">Connect Tailscale on your iPhone, then open this private address in Safari.</p><div class="phone-address">'+esc(status.origin)+'</div><div class="row"><button class="btn" data-phone-copy="'+esc(status.origin)+'">Copy address</button><button class="btn primary" data-phone-pair>Create pairing code</button></div>'+(pairing?'<div class="phone-code"><small>Enter this on your iPhone · expires in 5 minutes</small><strong>'+pairing.code.slice(0,4)+' '+pairing.code.slice(4)+'</strong></div>':'')+'<p class="note">After pairing, use Safari’s Share menu → Add to Home Screen. If the Home Screen app asks to pair again, create a fresh code here.</p><h3 class="phone-label">Paired devices</h3>'+deviceList+'<button class="btn danger" data-phone-disable>Turn off phone access</button>':'<p class="note">FOLKLET uses Tailscale to connect your computer and iPhone privately. Sign in to the same Tailscale account on both devices.</p><div class="phone-steps"><a href="https://tailscale.com/download/ios" target="_blank" rel="noopener noreferrer">1. Get Tailscale for iPhone ↗</a>'+instructions+'<button class="btn" data-phone-refresh>Check connection</button></div>'+setupDetails;
 modal('Connect iPhone','Your bots and chats, wherever you are.',content+'<p class="note phone-footer">Keep this computer awake with FOLKLET running. Closing the desktop window leaves FOLKLET in the system tray.</p>','phone');
...
function drawAccount(state){
 ...
 let body;
 if(signed&&state.error){body='<p class="note" role="alert">Could not refresh account status. ChatGPT was connected at the last successful check.</p><p class="note">'+esc(state.error)+'</p><p class="note">FOLKLET will check again automatically. Your sign-in has not been changed.</p><button class="btn" data-close-modal>Back to FOLKLET</button>';}
 else if(signed){accountLogin=null;body='<div class="account-connected"><span>✓</span><div><b>ChatGPT connected</b><small>'+esc(state.plan?state.plan.charAt(0).toUpperCase()+state.plan.slice(1)+' plan':'Your ChatGPT account')+'</small></div></div><p class="note">FOLKLET is ready. Its engine is included in this app, so you can use FOLKLET with the Codex desktop app closed or uninstalled.</p><p class="note">Usage follows your account’s limits. Your conversations and bot files stay in FOLKLET.</p><button class="btn primary" data-close-modal>Back to FOLKLET</button>';}
 else if(pending&&accountLogin){body='<p class="note">Continue in your browser and sign in to ChatGPT. Then return here; FOLKLET will connect automatically.</p><a class="btn primary account-login-link" href="'+esc(accountLogin.authUrl)+'" target="_blank" rel="noopener noreferrer">Continue to ChatGPT ↗</a><p class="note account-wait">Waiting for sign-in…</p><button class="btn quiet" data-account-cancel="'+esc(accountLogin.loginId)+'">Cancel sign-in</button>';}
...
async function updateAccountPanel(){
 ...
 catch(e){if($('modal').open&&modalType==='chatgpt-account'){accountKey='';modal('ChatGPT account','Built into FOLKLET.','<p class="note" role="alert">Could not refresh account status.</p><p class="note">'+esc(e.message)+'</p><p class="note">FOLKLET will check again automatically. Your sign-in has not been changed.</p><button class="btn" data-close-modal>Back to FOLKLET</button>','chatgpt-account');}}
...
const settingsUI=createSettingsUI({api,modal,refresh,getBots:()=>data.bots,accountPanel,md,onSource:openSource,onAppChange:()=>appChangesUI.open(),draftStorage,onDraftChange:()=>{persistDrafts();}});
async function openSource(source,loaded=false){
 let url;try{url=new URL(source,location.href);}catch{toast('This conversation link is invalid.');return;}
 if(url.origin!==location.origin||url.pathname!=='/'){toast('This source is outside this FOLKLET workspace.');return;}
 ...
 if((messageId||taskId)&&!message&&!loaded){try{data=syncClient.addHistory(await api('history?id='+encodeURIComponent(owner.id)+(messageId?'&message='+encodeURIComponent(messageId):'&task='+encodeURIComponent(taskId))));return openSource(source,true);}catch{toast('This source message is no longer available.');return;}}
...
document.addEventListener('click',async event=>{const button=event.target.closest('[data-load-history]');if(!button||button.disabled)return;button.disabled=true;const original=channel||selected,workspace=data.workspaceId;try{const owners=channel?data.bots.filter(b=>data.channels.find(c=>c.id===channel)?.members.includes(b.id)):[current()];for(const owner of owners.filter(b=>(b.messageCount||0)>b.messages.length)){const page=await api('history?id='+encodeURIComponent(owner.id)+'&before='+encodeURIComponent(syncClient.before(owner.id)));if(data.workspaceId!==workspace)return;data=syncClient.addHistory(page);}if((channel||selected)===original){const el=$('messages'),height=el.scrollHeight,top=el.scrollTop;renderChat();el.scrollTop=top+el.scrollHeight-height;}}catch(e){toast(e.message);}finally{button.disabled=false;}});
