...
async function api(route,body){let r;try{r=await fetch('/api/'+route,{method:body?'POST':'GET',headers:{'X-Crew-Token':window.CREW_TOKEN,'Content-Type':'application/json'},body:body?JSON.stringify(body):undefined});}catch{throw Error('FOLKLET cannot reach your host. Check your connection and keep FOLKLET running.');}if(r.status===401&&window.CREW_MOBILE)location.replace('/');let result;try{result=await r.json();}catch{throw Error('FOLKLET could not read the response. Check your connection and try again.');}if(!r.ok)throw Error(result.error||'Request failed');return result;}
