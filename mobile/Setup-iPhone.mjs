...
export function findTailscale({platform=process.platform,env=process.env,exists=executable}={}){
 if(!['darwin','linux'].includes(platform))throw new SetupError('Use Crew’s Windows iPhone setup on this computer.');
...
export function checkServeConfig(value,dns){
 ...
 for(const [host,enabled] of Object.entries(config.AllowFunnel||{}))if(host.endsWith(':'+servePort)&&enabled)throw new SetupError('Port 8443 is configured for public Funnel. Disable that endpoint in Tailscale before using it for private Crew access.');
 for(const foreground of Object.values(config.Foreground||{}))if(object(foreground)&&(foreground.TCP?.[servePort]||Object.keys(foreground.Web||{}).some(host=>host.endsWith(':'+servePort))))throw new SetupError('Tailscale port 8443 is already in use by a foreground service. Crew left it unchanged.');
 for(const host of Object.keys(config.Web||{}))if(host.endsWith(':'+servePort)&&host!==endpoint)throw new SetupError('Tailscale port 8443 already serves another app. Crew left it unchanged.');
 const web=config.Web?.[endpoint],tcp=config.TCP?.[servePort];
 if(web!==undefined){
  const handlers=web?.Handlers,handler=handlers?.['/'];
  if(!object(web)||Object.keys(web).some(key=>key!=='Handlers')||!object(handlers)||Object.keys(handlers).length!==1||!object(handler)||Object.keys(handler).length!==1||handler.Proxy!==target)throw new SetupError('Tailscale port 8443 already serves another app. Crew left it unchanged.');
 }
 if(tcp!==undefined&&(!object(tcp)||tcp.HTTPS!==true||Object.keys(tcp).some(key=>key!=='HTTPS')))throw new SetupError('Tailscale port 8443 is already in use by a TCP service. Crew left it unchanged.');
...
async function crewRequest(localPort,localToken,route,body){
 ...
 if(!response.ok)throw new SetupError('Crew could not save the phone connection. Keep Crew open, refresh it, and run setup again.');
...
export async function setupPhone({localPort,localToken,mobilePort=4320},{platform=process.platform,env=process.env,exists=executable,runCli=cli,requestCrew=crewRequest}={}){
 try{
  if(!Number.isInteger(localPort)||localPort<1||localPort>65535||!/^[a-f0-9]{64}$/.test(localToken||''))throw new SetupError('Open Crew and start phone setup from its Connect iPhone dialog.');
  if(mobilePort!==4320)throw new SetupError('Phone setup requires Crew’s standard companion port 4320. Restart Crew without a custom companion port.');
  ...
  if(!checkServeConfig(after,dns))throw new SetupError('Tailscale did not confirm Crew’s private connection. Update Tailscale and run setup again.');
 ...
 }catch(error){return {status:'failed',error:error instanceof SetupError?error.message:'Phone setup could not finish. Keep Crew and Tailscale open, then retry.',...(error instanceof SetupError&&error.actionUrl?{actionUrl:error.actionUrl}:{})};}
}
...
export class PhoneSetupRunner {
 ...
 start(input){
  ...
  try{
   ...
   child.on('error',()=>finish({status:'failed',error:'The phone setup helper could not start. Restore the complete Crew package and retry.'}));
  ...
  }catch{finish({status:'failed',error:'The phone setup helper could not start. Restore the complete Crew package and retry.'});}
  return this.state;
 }
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
 ...
 catch{result={status:'failed',error:'Open Crew and start phone setup from its Connect iPhone dialog.'};}
