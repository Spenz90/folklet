...
function protectSession(crewSession){
 ...
 crewSession.on('will-download',(event,item,contents)=>{
  ...
  const name=path.basename(item.getFilename()).replace(/[\u0000-\u001f\u007f]/g,'_')||'Crew download';
  item.setSaveDialogOptions({title:'Save from Crew',defaultPath:path.join(app.getPath('downloads'),name)});
 });
}

function createWindow(){
 const crewSession=session.fromPartition('persist:crew-window');protectSession(crewSession);
 mainWindow=new BrowserWindow({title:'Crew',width:1280,height:860,minWidth:780,minHeight:540,show:false,backgroundColor:'#070707',icon:path.join(appRoot,'icons','crew-512.png'),
 ...
 mainWindow.loadURL(HOME_URL).catch(()=>dialog.showErrorBox('Crew could not open','Use View → Reload to reconnect to the local app.'));
}

function createMenus(){
 const quitItem={label:'Quit Crew',accelerator:process.platform==='darwin'?'Command+Q':'Control+Q',click:()=>app.quit()};
 const installItem={id:'install-browser',label:'Install browser',click:()=>void downloadBrowser()};
 const template=[{label:'Crew',submenu:[{label:'Show Crew',click:showWindow},installItem,{type:'separator'},quitItem]},
  ...
  {label:'Window',submenu:[{role:'minimize'},{label:'Show Crew',click:showWindow}]}];
 Menu.setApplicationMenu(Menu.buildFromTemplate(template));
 try{const icon=nativeImage.createFromPath(path.join(appRoot,'icons','crew-192.png')).resize({width:20,height:20});tray=new Tray(icon);tray.setToolTip('Crew');trayMenu=Menu.buildFromTemplate([{label:'Open Crew',click:showWindow},installItem,{type:'separator'},quitItem]);tray.setContextMenu(trayMenu);tray.on('click',showWindow);}catch{/* The application menu and taskbar remain available without a tray. */}
...
async function downloadBrowser(){
 ...
 setInstalling(true);
 try{await installBrowser({appRoot,browserRoot,signal:browserInstall.signal});if(!quitting&&!quitPending)await dialog.showMessageBox(mainWindow,{type:'info',title:'Browser ready',message:'Crew’s browser is installed.',detail:'Your bots can now open their own browser.',buttons:['Done']});}
...
async function quitCrew(){
 ...
 catch(error){quitPending=false;showWindow();dialog.showErrorBox('Crew is still running',error.message);}
}
