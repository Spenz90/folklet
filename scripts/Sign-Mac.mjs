...
export async function signMacArchive({archive,platform}={}){
 ...
 if(!archive.endsWith('.zip')||!fs.statSync(archive,{throwIfNoEntry:false})?.isFile())throw Error('Choose an existing Crew Mac ZIP archive.');
 ...
 try{
  ...
  const app=path.join(stage,'Crew.app');
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
 ...
 for(let i=0;i<args.length;i++){const name={'--archive':'archive','--platform':'platform'}[args[i]];if(!name||!args[i+1])throw Error('Use --archive <Crew-Mac.zip> --platform darwin-arm64|darwin-x64');options[name]=args[++i];}
