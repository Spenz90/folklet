...
export class Skills{
 ...
 importSkill(input){
  ...
  if(!value||typeof value!=='object'||Array.isArray(value)||value.format!=='crew-instruction-skill'||value.version!==1||Object.keys(value).some(key=>!['format','version','skill'].includes(key))||!value.skill||typeof value.skill!=='object'||Array.isArray(value.skill)||Object.keys(value.skill).some(key=>!['title','whenToUse','steps','examples','checklist'].includes(key)))throw Error('Import only FOLKLET instruction-skill JSON. Executables, files, permissions and activation settings are not imported.');
