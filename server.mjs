...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='GET'&&url.pathname==='/auth/openrouter/callback'){
  ...
  return res.end('<!doctype html><html><title>Crew connection</title><body style="font:18px system-ui;background:#111;color:#eee;padding:64px;max-width:650px"><h1>'+(ok?'OpenRouter connected':'Connection not completed')+'</h1><p>'+(ok?'Return to Crew. You can close this tab.':'The sign-in expired or was not completed. Return to Crew and start a new connection.')+'</p></body></html>');
 }
