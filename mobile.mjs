...
export class MobileAccess {
 ...
 async handle(req,res){try{
  ...
  if(req.method==='POST'&&url.pathname==='/unpair'&&paired){if(!safeEqual(req.headers['x-crew-token'],csrfFor(paired.secret)))return json(res,403,{error:'Refresh FOLKLET before disconnecting.'});this.revoke(paired.id);return json(res,200,{ok:true},{'Set-Cookie':`${cookieName}=; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=0`});}
  if(!paired&&!publicAssets.includes(url.pathname)){
   ...
   return json(res,401,{error:'Pair this device again from FOLKLET on your PC.'});
  }
  if(url.pathname.startsWith('/api/vault-')||url.pathname.startsWith('/api/backup-')||url.pathname.startsWith('/api/catalog-')||url.pathname==='/api/usage-configure'||url.pathname==='/api/push-configure'||url.pathname==='/api/update-check'||url.pathname.startsWith('/api/mobile')||url.pathname.startsWith('/api/account')||(url.pathname.startsWith('/api/provider')&&!(req.method==='GET'&&['/api/providers','/api/provider-models'].includes(url.pathname)))||url.pathname.startsWith('/api/notification-')||url.pathname.startsWith('/api/integration-')||url.pathname.startsWith('/api/app-change')||url.pathname.startsWith('/api/plugin')||url.pathname==='/api/native-enable'||url.pathname==='/api/shutdown'||url.pathname==='/health')return json(res,403,{error:'Manage FOLKLET from your host computer.'});
  const api=url.pathname.startsWith('/api/');
  if(api&&(!paired||!safeEqual(req.headers['x-crew-token'],csrfFor(paired.secret))))return json(res,403,{error:'Refresh FOLKLET to reconnect.',code:'CONNECTION_EXPIRED'});
  if(!['GET','POST'].includes(req.method)||(!api&&!['/','/app.js','/connection-state.mjs','/hosting-ui.mjs','/draft-storage.mjs','/sync-client.mjs','/reliability-ui.mjs','/markdown.mjs','/settings-ui.mjs','/model-settings-ui.mjs','/review-ui-common.mjs','/skills-ui.mjs','/learning-review-ui.mjs','/recall-ui.mjs','/integrations-ui.mjs','/notifications-ui.mjs','/routine-policy-ui.mjs','/fallback-ui.mjs','/app-changes-ui.mjs','/plugins-ui.mjs','/style.css',...publicAssets].includes(url.pathname)))return json(res,404,{error:'Not found'});
  if(req.method==='POST'&&!api)return json(res,404,{error:'Not found'});
  const payload=req.method==='POST'?await body(req):undefined;
  const upstream=await fetch(`http://127.0.0.1:${this.localPort}${url.pathname}${url.search}`,{method:req.method,headers:{'X-Crew-Token':this.localToken,...(paired?{'X-Crew-Device':paired.id}:{}),'Content-Type':'application/json'},body:payload,redirect:'error',signal:AbortSignal.timeout(api?120000:15000)});
  ...
  if(url.pathname==='/'&&paired){const original=await upstream.text(),marker="window.CREW_TOKEN='"+this.localToken+"';";if(!original.includes(marker))throw Error('FOLKLET needs an update before phone access can continue.');const content=original.replace(marker,"window.CREW_TOKEN='"+csrfFor(paired.secret)+"';window.CREW_MOBILE=true;");if(content.includes(this.localToken))throw Error('FOLKLET could not establish a private phone session.');res.writeHead(upstream.status,headers);return res.end(content);}
